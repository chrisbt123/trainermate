  ...
  if (/\/terms\.html$/i.test(window.location.pathname)) {
    ...
    Array.prototype.forEach.call(document.querySelectorAll("h2"), function (heading) {
      if (/^7\. Plans and subscriptions/i.test(heading.textContent || "")) {
        ...
        if (heading.nextElementSibling) heading.nextElementSibling.textContent = "TrainerMate Free includes limited features. TrainerMate Pro is available as a £5 one-month pass with no renewal, a £5 monthly subscription, or a £40 annual subscription. Recurring plans renew until cancelled. Cancellation stops the next renewal; access continues through the period already paid for and then automatically returns to Free without deleting saved settings. Cancellation does not itself refund the current paid period. Payments and refunds are processed securely by Stripe. Your statutory rights are not affected.";
      }
